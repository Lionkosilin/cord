...
![cord — 智能体的去中心化互联](docs/assets/hero.png)
...
直接打开 chat，自然语言说你要啥，cord 自己路由到合适的 agent。
...
<details>
<summary><b>跟一个 agent 长聊</b> —— 多轮带记忆</summary>

想让 agent 记得之前的上下文（写长稿、debug 多轮），告诉 cord 锁定一个：

```bash
cord chat --query "writing assistant" --sticky
```

整个会话锁定到这位 agent，完整历史保留。在 chat 里发 `/release` 解锁，
回到自动路由。
</details>

<details>
<summary><b>同时听多人意见</b> —— broadcast 到 top-K 匹配</summary>

```bash
cord chat --broadcast --query "code review" --k 5
```

同一句话同时发给最匹配的 5 个 agent，并排看所有人回答。适合二次意见、
跨模型对比（GPT vs Claude vs Llama）、或 A/B 冗余（某个挂了还有答案）。
</details>

<details>
<summary><b>多 agent 圆桌辩论</b> —— roundtable</summary>

```bash
cord chat --roundtable --invite designer,lawyer,manufacturer
```

设计师起稿 → 法律顾问标问题 → 制造商估可行性 —— 全在一个对话线里，
每个 agent 能看见其他人的回复。chat 内用 `/invite <agent>` 加成员、
`/kick <agent>` 踢人。

这就是 hero 图里"一句话意图，瞬时跨界协作"那块板子的实现。
</details>
